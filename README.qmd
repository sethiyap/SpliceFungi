...
```{r mev_1h_splicing, echo=TRUE, message=FALSE}
...
```{r bs181_1h_splicing, echo=FALSE, message=FALSE}

ref_path <- "/Users/pooja/Documents/CDK7_project/SpliceFungi/Reference"

pb_path_bs_1h <- file.path("/Users/pooja/Documents/CDK7_project/SpliceFungi", "spliceWiz_process_1h_BS181/")

control = "WT_1h"

treatment = "WT_bs_1h"

se_bs_1h <- SpliceFungi::get_ASE(reference_path = ref_path, processed_bam_path = pb_path_bs_1h, control = control, treatment = treatment, replicates = 3)

se.filtered_bs_1h <- se_bs_1h[SpliceWiz::applyFilters(se_bs_1h),]

res_edgeR_bs_1h <- SpliceFungi::get_DE_ASE(se_filtered_object =  se.filtered_bs_1h, control = control, treatment = treatment, plot_volcano = FALSE)

raw_value_bs_1h <- SummarizedExperiment::assay(se.filtered_bs_1h, "Included") %>% tibble::as_tibble(rownames = NA)

# splicing stats

SpliceFungi::splicing_statistics(se_filtered_object = se.filtered_bs_1h, res_edgeR = res_edgeR_bs_1h)
...
```{r bs181_5h_splicing, echo=TRUE, message=FALSE}
...
```{r bs_5h_splicing_stats, echo=TRUE, message=FALSE}
...
```{r bs_5h_IR_events, echo=TRUE, message=FALSE, fig.height=6, fig.width=12}
...
```{r GO_analysis_bs, echo=TRUE, message=FALSE}
